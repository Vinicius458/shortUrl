...
const makeSut = (): SutTypes => {
  ...
  const sut = new DbLoadAccountByToken(decrypterStub, userRepositoryStub);
...
describe("DbLoadAccountByToken UseCase", () => {
  ...
  test("should return the account id if UserRepository returns a valid account", async () => {
    const { sut, decrypterStub, userRepositoryStub } = makeSut();
    jest
      .spyOn(decrypterStub, "decrypt")
      .mockResolvedValueOnce("decrypted_token");
    jest
      ...
      .mockResolvedValueOnce("account_id");
